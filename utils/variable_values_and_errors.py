...
def get_error(df,true_value,function,error_type,vel_x_var=None,vel_y_var=None,montecarloconfig=None,bootstrapconfig=None,tilt=False, absolute=False, R_hat = None):
    ...
    if error_type == "bootstrap":
        ...
        std_low,std_high,_ = CV.get_std_bootstrap(function=function, vx=vx, vy=vy, tilt=tilt, absolute=absolute, R_hat=R_hat,bootstrapconfig=bootstrapconfig)

        return std_low,std_high
    
    elif error_type == "MC":
        std_low,std_high,*_ = CV.get_std_MC(df=df, true_value=true_value,function=function, montecarloconfig=montecarloconfig, vel_x_var=vel_x_var, vel_y_var=vel_y_var, \
...
def get_all_variable_values_and_errors(df_vals,vel_x_var,vel_y_var,full_map_string_list,df_errors=None,montecarloconfig=None,bootstrapconfig=None,\
    ...
    if "mean_vx" in full_map_string_list:
        ...
        if verbose and error_type == "MC" and montecarloconfig.perturbed_var == "d" and vel_x_var=="r":
